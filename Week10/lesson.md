...
Last week, we broke the dialog in Much Ado About Nothing into [a nicely formatted JSON file](../Week09/answers/MAAN_dialog.txt). Let's make use of that now.
...
Just for NLTK, we need to prime the module by downloading a key piece of data. So we should jump into the Python interactive interpreter and just run these two lines:
...
We only need to do this once for our environment.
...
First, we need to tokenize the dialog (i.e. break up the dialog into words) using strip. And then let's remove *stopwords*, which are common English words like articles that don't provide much insight in this sort of analysis. 
