...
If you're using Linux, I'm going to assume that you know what you're doing. Please let me know if this is a faulty assumption. Install Python 3, pip, and uv.
...
We are not going to use pipenv, so don't do anything in the "Pipenv & Virtual Environments" section.
...
In the terminal, run: `brew install uv`

At the end of this step, you should be able to enter the command `uv --version` and have it not return an error.
...
### Step 1-W: Install Homebrew

We're going to install Homebrew, a package manager originally made for Mac but now [ported to Linux/WSL](https://docs.brew.sh/Homebrew-on-Linux). This will simplify installing and updating other tools.

### Step 2-W: Install Python and Pip
...
* Install Debian development tools, openSSL, and Python extension headers, and more run: `sudo apt-get install build-essential libssl-dev libffi-dev python3-dev procps curl file git`

### Step 3-W:  Install uv

In your terminal, run: `brew install uv` 

At the end of this step, you should be able to input the command `uv --version` and have it not return an error.
...
### Step X-W: Switch from Bash to Zsh (optional)
